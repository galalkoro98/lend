...
function createItem(name: string, description: string, borrowerName: string, lendingDate: string, reminderDate: string) {
  ...
  const newItemRef = push(ref(db, 'items')); // Generate a unique ID for the new item
  const newItemId = newItemRef.key; // Get the unique ID generated by Firebase
  set(newItemRef, {
    id: newItemId,
    name: name,
...
const ModalExample = ({ onDismiss }: { onDismiss: (e?: OverlayEventDetail) => void }) => {
  ...
  const handleConfirm = () => {
    if (!name || !description || !borrowerName || !lendingDate || !reminderDate) {
      ...
      if (itemId) {
        setToastMessage(`Item ${name} created successfully`);
        ...
        setToastMessage('Error creating item');
        setToastColor('danger');
        setShowToast(true);
      }
    }
